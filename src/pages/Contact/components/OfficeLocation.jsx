...
import { getOfficeDetails } from "../../../lib/cms";
  ...
  directions: [
    {
      ...
      instruction: "Prendre la nationale N1 vers le centre-ville, tourner à droite au rond-point d'Almamya, continuer sur 500m",
      duration: "25 minutes en voiture",
    },
    {
      landmark: "Du Palais du Peuple",
      instruction: "Prendre l'Avenue de la République vers le nord, tourner à gauche au carrefour d'Almamya",
      duration: "10 minutes en voiture",
    },
    {
      landmark: "Transport en Commun",
      instruction: "Prendre les bus ligne 12 ou 15 jusqu'à l'arrêt Almamya, marcher 200m vers la rue KA-028",
...
const OfficeLocation = () => {
  ...
  const facilities  = office.facilities  || STATIC_OFFICE.facilities;
  const directions  = office.directions  || STATIC_OFFICE.directions;
  const hours       = office.hours       || STATIC_OFFICE.hours;
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-soft overflow-hidden">
              <div className="h-80 relative">
                <iframe
                  ...
                  src={`https://www.google.com/maps?q=${office.coordinates}&z=16&output=embed`}
                  className="border-0"
                />
              </div>
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-4 text-sm">
                      <a href={`tel:${office.phone}`} className="flex items-center space-x-2 text-primary hover:text-primary/80 transition-colors">
                        <Icon name="Phone" size={16} />
                        <span>{office.phone}</span>
                      </a>
                      <a href={`mailto:${office.email}`} className="flex items-center space-x-2 text-primary hover:text-primary/80 transition-colors">
            ...
            <div className="bg-white rounded-xl p-6 shadow-soft">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="text-xl font-heading font-semibold text-secondary">Business Hours</h3>
              </div>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Lundi - Vendredi</span>
                  <span className="font-medium text-secondary">{hours.weekdays?.replace("Lundi - Vendredi: ", "") || "8:00 - 18:00"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Samedi</span>
                  <span className="font-medium text-secondary">{hours.saturday?.replace("Samedi: ", "") || "9:00 - 14:00"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Dimanche</span>
                  <span className="font-medium text-secondary">{hours.sunday?.replace("Dimanche: ", "") || "Fermé"}</span>
                </div>
                {office.timezone && (
                  <div className="pt-2 border-t">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Timezone</span>
                      <span className="font-medium text-secondary">{office.timezone}</span>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-soft overflow-hidden">
              ...
              <div className="p-4 border-t">
                <h3 className="font-semibold text-secondary mb-2">Our Modern Workspace</h3>
                <p className="text-sm text-muted-foreground">State-of-the-art facilities designed for innovation, collaboration, and client meetings in the heart of Conakry.</p>
              </div>
            </div>

            {/* Facilities */}
            <div className="bg-white rounded-xl p-6 shadow-soft">
              <h3 className="text-xl font-heading font-semibold text-secondary mb-4">Office Facilities</h3>
              <div className="grid grid-cols-2 gap-4">
                {facilities.map((facility, index) => (
                  <div key={index} className="flex items-center space-x-3">
                    <div className="w-8 h-8 bg-primary/10 rounded-full flex items-center justify-center">
                      <Icon name={facility.icon} size={16} color="var(--color-primary)" />
                    </div>
                    <span className="text-sm text-secondary">{facility.label}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Directions */}
            <div className="bg-white rounded-xl p-6 shadow-soft">
              <h3 className="text-xl font-heading font-semibold text-secondary mb-4">How to Find Us</h3>
              <div className="space-y-4">
                {directions.map((direction, index) => (
                  <div key={index} className="flex items-start space-x-3">
                    ...
                    <div>
                      <h4 className="font-medium text-secondary text-sm mb-1">{direction.landmark}</h4>
                      <p className="text-xs text-muted-foreground mb-1">{direction.instruction}</p>
                      <span className="text-xs text-primary font-medium">{direction.duration}</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="mt-12 bg-primary/5 rounded-xl p-6 border border-primary/20">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="font-semibold text-secondary">Emergency Support</h3>
                <p className="text-sm text-muted-foreground">For existing clients with urgent technical issues</p>
              </div>
            </div>
            <div className="flex items-center space-x-4">
              <a href={`tel:${office.phone}`} className="flex items-center space-x-2 text-primary hover:text-primary/80 transition-colors">
